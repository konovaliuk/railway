    ...
    <form class="form-inline">
        <div class="input-group">
            ...
            <select id="usersOnPage" class="form-control" onchange="usersOnPageChange();">
                ...
                <option value="0"  ${usersOnPage == 0 ? 'selected' : '' } ><fmt:message key="showUsers.option.all" bundle="${lang}"/></option>
            </select>
                ...
                <tbody>
                    <c:forEach items="${users}" var="user" varStatus="loopCount" >
                        <tr>
                            ...
                            <td align="center">
                                <SELECT class="form-control" onchange="userInfoChange(${user.id});" id="user_role_id_${user.id}" value="${user.roleId}">
                            ...
                            <td align="center">
                                <input onchange="userInfoChange(${user.id});" id="is_user_banned_id_${user.id}" type='checkbox' ${user.banned ? 'checked' : '' } >
                            </td>
                            <td align="center">
                                <button class='btn btn-default' id="choose_btn_id_${user.id}" onclick='updateUser(${user.id});'><i class="fa fa-pencil-square-o"></i></button><!-- make local or pictogram -->
                            </td>
                        <tr>
                    </c:forEach>
                </tbody>
